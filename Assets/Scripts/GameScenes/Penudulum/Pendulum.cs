...
//        this.Bob.transform.position = this.PendulumUpdate(this.Bob.transform.position, Time.deltaTime);
        ...
        {
            this.Line.SetPosition(0, this.Pivot.transform.position);
            this.Line.SetPosition(1, this.Bob.transform.position);
        }
        ...
        Vector3 PendulumUpdate(Vector3 currentStatePosition, float deltaTime)
        {
            ...
            if (distanceAfterGravity > this.ropeLength || Mathf.Approximately(distanceAfterGravity, this.ropeLength))
            {
                ...
                this.tangentDirection =
                    (-1f * Vector3.Cross(this.tensionDirection, this.pendulumSideDirection)).normalized;
            ...
            return this.GetPointOnLine(pivot_p, currentStatePosition + movementDelta,
                distance <= this.ropeLength ? distance : this.ropeLength);
        }
